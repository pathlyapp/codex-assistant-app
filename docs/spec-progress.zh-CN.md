...
- 发现旧“一键还原”默认停止并卸载 ChatGPT、移除配置和删除全部助手数据，违反
  `LIFE-002/LIFE-003`。前端和核心已移除 `factory_reset`，不再直接调用
  `Remove-AppxPackage`。
- 新增 `LifecycleStatusV1/LifecycleActionResultV1`，把助手程序、受管 Codex 配置、
  助手数据和 ChatGPT 官方应用拆成四个独立对象。助手卸载默认明确保留后三项。
- 恢复配置使用新事务，只移除助手管理块并保留其他 TOML；删除数据在受管配置仍存在
  时由核心返回 `LIFECYCLE_DATA_IN_USE`。ChatGPT 只交给系统应用管理再次确认。
- 本机格式、严格 Clippy、前端语法和 Rust 全量测试通过：67 通过、0 失败、1 个
  本地 Ollama live test 忽略；Windows ARM64/x64 目标结果相同，两个 NSIS 候选
  均构建成功。
- ARM64 原生与 x64 兼容层均通过静默安装不自启、首次响应、单实例和完整 UI E2E。
  生命周期 E2E 验证受管配置存在时拒绝删数据、事务化恢复保留无关 TOML、页面删除
  助手数据、ChatGPT 保持可信、交互卸载回执可见且助手退出、默认 NSIS 卸载保留
  配置/数据/ChatGPT，并成功重装同一 SHA256 候选。
- ARM64 候选为 5,623,641 字节，SHA256
  `515e135549ca02715a557f7a695bc96405cb5433a8f2ada9fc2ae5c0c568337c`；
  x64 候选为 6,007,909 字节，SHA256
  `46eb890c23cabfd69bac0daf785392453114dd77b263ab2ff1c2b97bb92cf427`。
  VM 最终恢复 ARM64 原生候选并重新完成受控 Router 配置；真实 x64 和签名 macOS
  候选仍是独立门禁。
