...
| 开发基线 | `main@501f071`，应用版本 `0.9.0` | M7 `WP-700` 已合并；包含 M1-M6、Router 断开恢复和发布分级门禁 |
...
开发主线后续清单使用 schema v2 发布分级。它不修改上表的历史 `v0.8.4` 证据：

- 当前流水线只允许 `internal-test`，并写入 `customerReady=false`。
- 未提供渠道或请求 `customer` 均使构建失败。
- SHA256 继续生成，但明确不等于代码签名或发布者身份。
- 客户渠道必须等待 Windows/macOS 签名、公证和签名清单验证。
